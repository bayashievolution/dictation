...
これは**やっさんの報告に対する最有力の仮説**であって、当該エラーの原因だと確定はしていない。
確定させるには、そのノートの文字起こしが**末尾まで入っているか**を、アプリ側に残っている
同じタブと突き合わせる必要がある（失敗したタブは閉じない仕様なので突き合わせられる）。
...
- 2026-08-31 (v0.15.2): 作った直後のノートへの追記で 404 が返ったとき再試行するよう修正（ノートが途中で切れる不具合）／404 のメッセージを呼び先で出し分け／失敗ログに保存先を記録
