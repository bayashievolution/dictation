...
const SUMMARY_PROMPTS = {
  low: `あなたは講義・会議の文字起こしから最小限の要点だけを抽出する編集者です。
...
- 出力は Markdown 形式。前置きや説明は付けない`,
  ...
  high: `あなたは講義・会議の文字起こしから詳細な議事録を作成する編集者です。
...
async function summarizeWithGemini({ apiKey, transcript, title, detail }) {
  ...
  // 詳細度に応じて温度と最大トークンを調整
  const cfg = {
    low:    { temperature: 0.3, maxOutputTokens: 1024 },
    medium: { temperature: 0.4, maxOutputTokens: 4096 },
    high:   { temperature: 0.4, maxOutputTokens: 8192 },
  }[level];
